  ...
  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    ...
    <table class="table table-bordered">
        ...
        while ($row = mysqli_fetch_assoc($customer_query)) {
            ...
            SELECT quotation_items.quantity, quotation_items.price, items.name, items.id AS itemID
                ...
                while ($item = mysqli_fetch_assoc($get_items)) {
                    ...
                    while ($all_row = mysqli_fetch_assoc($all_items)) {
                        ...
                        foreach ($all_row as $key => $itemData) {
                            // foreach ($all_row as $itemData) {
                            // echo "$key: $itemData";
                            // echo implode(' ', $itemData);
                    ...
                    $quote_item_data = "{quotation_item_id: " . $quote_item_id . ",quantity: " . $item['quantity'] . ", price: " . $item['price'] . "}";
                    $quote_select_items = implode('/', $quote_select_item);
                    // echo var_dump($quote_select_items);
                    ...
                    <tr>
                        ...
                        <td>
                                    ...
                                    <form action="add_items.php" method="post" class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input name="quotation_qty" value="" id="quote_quantity_update"
                                                    class="form-control border border-success bg-light"
                                                    (change)="enterDirectRowNumberResponse(data,$event.target.value)" required />
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label"><strong>Price</strong></label>
                                                <input name="quotation_price" value="" id="quote_price_update"
                                                    class="form-control border border-success bg-light" required />
                                            </div>
                                        </div>
                            ...
                            <a href="?did=<?php echo $item['itemID']; ?>" class="btn btn-sm btn-danger"
...
<script>
    function setModelData(row, item_data) {
        ...
        for (var select_item in quote_select_items) {
            ...
            quote_item_update.onchange = function () {
                var my_select = event.target;
                my_select.setAttribute('selected');
            }
